...
function wpsa_module1_ttfb( $tested_url, $debug_log, $results_log ) {
    ...
    // 2) Instead of echoing inline <script>, attach via wp_add_inline_script()
    $js_display = sprintf(
    ...
    // Instead of echoing inline JS, enqueue via wp_add_inline_script
    $js_hide = 'jQuery(function($){ $("#running-test").hide(); });';
    wp_add_inline_script( 'wpsa-admin-scripts', $js_hide );
